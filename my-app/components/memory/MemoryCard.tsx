...
export default function MemoryCard({ memory, onPromoted }: Props) {
  ...
  const handlePromote = async () => {
    ...
    try {
      ...
      if (!token) return;
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === "production" ? "https://atlas-1sr4.onrender.com" : "http://127.0.0.1:8000")}/memory/promote`, {
        method: "POST",
        headers: { 
